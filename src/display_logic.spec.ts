...
describe('display logic', () => {
  ...
  describe('InputBlockLogic', () => {
    test('initial doesn\'t trigger', () => {
      ...
      const f = watch_f((_: Constraint[] | undefined) => {})
      block.on_ready(f.f)
    ...
    test('gradually filling in entries triggers each time', () => {
      const block = make_block()
      const f = watch_f((_: Constraint[] | undefined) => {})
      block.on_ready(f.f)
    ...
    test('removing 1 of multiple errors', () => {
      const block = make_block()
      const f = watch_f((_: Constraint[] | undefined) => {})
      block.on_ready(f.f)
      expect(f.calls).toEqual({ tag: 'not-called' })
    ...
    describe('focus', () => {
      ...
      test('adding single', () => {
        const block = new InputBlockLogic(parse_constraint, () => undefined)
        const input = block.insert_input_after(undefined)
        const on_focus = watch_f((_: boolean) => {})
      ...
      test('adding multiple', () => {
        ...
        const on_input0_focus = watch_f((_: boolean) => {})
        ...
        const on_input1_focus = watch_f((_: boolean) => {})
        ...
        const on_input2_focus = watch_f((_: boolean) => {})
    ...
    //   })
    // })
    test('introducing an error in a bunch of parseables', () => {
      const block = make_block()
      const on_ready = watch_f((_: Constraint[] | undefined) => {})
      block.on_ready(on_ready.f)
      block.set_fields([
  ...
  describe('BatchInputLogic', () => {
    test('initial', () => {
      ...
      const on_sync = watch_f((_: boolean) => {})
    ...
    test('a bunch of new parseable lines', () => {
      const block = make_block()
      const batch = new BatchInputLogic(block)
      const on_sync = watch_f((_: boolean) => {})
      batch.on_sync(on_sync.f)
    ...
    test('inserting in parent desyncs', () => {
      const block = make_block()
      const batch = new BatchInputLogic(block)
      const on_sync = watch_f((_: boolean) => {})
      batch.on_sync(on_sync.f)
      batch.send()  // increment sync count by 1!

      block.insert_input_after(undefined)
      expect(on_sync.calls).toEqual({ tag: 'called-with', input: false, count: 2 })
    })
    test('editing input in parent desyncs', () => {
      const block = make_block()
      const batch = new BatchInputLogic(block)
      const on_sync = watch_f((_: boolean) => {})
      batch.on_sync(on_sync.f)
    ...
    test('removing input in parent desyncs', () => {
      const block = make_block()
      const batch = new BatchInputLogic(block)
      const on_sync = watch_f((_: boolean) => {})
      batch.on_sync(on_sync.f)
